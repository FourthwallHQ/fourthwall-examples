 ...
 * for a shop using the local app HMAC key so you can open the *real* settings
...
export async function GET(request: Request): Promise<Response> {
  ...
  if (!appId || !secret) {
    return Response.json(
      { error: 'set NEXT_PUBLIC_FOURTHWALL_APP_ID and FOURTHWALL_APP_HMAC_KEY' },
      { status: 500 },
    );
  }
  ...
  // Base64 contains +/= — URLSearchParams encodes them so the param round-trips.
  const qs = new URLSearchParams({ shop_id: shopId, hmac, timestamp }).toString();

  return Response.json({ url: `${baseUrl}/?${qs}` });
}
